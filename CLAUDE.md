...
- [x] **推廣 landing page + 宣傳截圖管線**(2026-07 第九輪,用戶四選一裁定:首頁 landing/暗色魔法風/YouTube 佔位/純英文):
  ...
  - **landing 視覺升級**(第九輪後續,用戶看了 Spline Remix 想要但要課金 → 純 SVG+vanilla JS 自製,Opus 開規格):①捲動進場 fadeInUp(IntersectionObserver+`.reveal`,特色卡 stagger)、特色卡 hover 紫金光暈、hero 標題呼吸光、Play 鈕 hover 脈衝;②**紫黃晶寶石互動層**(`#gemField`):10 種經典切型俯視線稿(Round/Radiant/Princess/Pear/Oval/Marquise/Heart/Emerald/Cushion/Asscher,SVG `<defs>` 手繪 path)散佈頁面兩側 16 顆、緩慢浮動,滑鼠 250px 半徑內從半透明白漸變成**紫黃晶 Ametrine**(上紫 #9b59b6 下金 #d4a017,`linearGradient`)+輕轉 22°,離開 0.8s 褪回;rAF 節流,觸控裝置直接不啟用追蹤(靜態白)。**踩過的坑:`<use>` 複製體在 shadow DOM,外部 CSS 選不到子元素**——著色只能靠 `<use>` 上的可繼承屬性(fill/stroke),漸層以各封閉形自身 bbox 取範圍反而做出「每個刻面各自紫→金」的分帶效果。
  - **三張爆點卡中二特效**(用戶給參考影片裁定,`.gitignore` 掉的 `中二上修/` 資料夾,分析法=preview 瀏覽器 video→canvas 抽幀;色系統一紫白):①Blind cutting=**斬擊光痕+玻璃碎裂**(Discord「支離破碎」風:slash 掃過→SVG polyline 裂痕 `pathLength=1` dashoffset 畫入→火花 `--dx/--dy` 飛散+卡片 shake);②Real faceting charts=**異世界演算符文橫向漂流**(用戶點名要橫的不要直的、公式計算感——內容全是引擎真公式 d=r·sinθ+h·cosθ/t⟼96−t 等,4 條 `.rl` marquee 常駐 opacity .24,hover 增亮 .7);③Overdramatic appraisal=**B+→A+→S+ 評級連段**(0.05s/0.5s/1s 依序 stamp 進場,S+ 66px 白紫光+爆圈)。觸發=hover/點擊/首次捲進視野自動演一次(手機也看得到);`.fx` class 3.4s 後自動卸掉(不卸符文卡會永久亮)。**favicon 同輪上線**(用戶自備 SVG+PNG 備援,兩個 HTML 都有 link)。驗證技巧:`getAnimations({subtree:true}).forEach(a=>a.pause())` 凍結動畫中格截圖(注意 play() 的 3.4s 自動卸 .fx 會清掉凍結,要等自動播完再手動加)。
  - **第二輪回饋修正**(用戶新參考):①卡1 蜘蛛網裂痕嚇人 → 改**優美雙斬**(兩道反向光痕交叉出 X+光瓣柔焦慢飄,水果忍者式獲好評保留延伸);②卡3 B+ 撞到 ⚖ icon → 移右上(`right:14%`),A+/S+ 不動;③**影片框紫焰**(用戶埋很久的願望,KPop 獵魔女團火牆參考但色彩改頁面紫白統一):`.vWrap` 包影片框,`.flames`=14 支火舌(`--fw/--fh/--fd/--fdl` 參數化,`fxFlick` 交錯搖曳)+**連續火焰基底帶**(`.fb.ft/.fl/.fr/.fbm` 四邊,把火舌接成一面火牆——只有獨立火舌會像分離圓柱,基底帶才是火牆感的關鍵)+`.fglow` 輝光呼吸,整層 `blur(7px)`,常駐動畫;手機降 blur+透明度,reduced-motion 全關。
  - **視覺修正(用戶回報線稿感太重會遮字,原參考是 3D 渲染)**:改成**填色刻面**風——每個刻面 polygon 把 `fill-opacity`(.12–.5)烘進 defs 資料模擬 3D 面光影,線條降到極淡(stroke α.22),`.base` 整體 opacity .5 呈磨砂玻璃,加 drop-shadow 浮起;染色時 base 退到 .12、tint 全彩。**防遮字三段**:<1250px 整層 opacity .5、<1100px 隱藏靠內側四顆(`.gem.mid`)、<820px 整層 .3。
  - **第4張 feature card + 過時文案修正**(2026-07,用戶:遊戲內時間魔法回溯是後來因為朋友體感需求才加的,原始設計是硬派 no-undo,landing page 兩處文案還停在舊行為):`<h2>Real machine, real cuts, no undo</h2>`→`mostly permanent`,卡1「Blind cutting」拿掉「Every cut is permanent」(跟卡4重複且過時);新增第4張純文字卡(無 fx 動畫,`.featGrid` 是 auto-fit grid 所以4張不用調版面)「Time-magic rewind」,文案刻意保持硬派調性(強調「有限3次」不是「隨便回溯」);`.reveal.in .feat:nth-child(4)` 補 stagger delay .45s。同時在 How it works 補一段 `#dev` 稽核工具(100% facet match / 0.0° deviation)撐硬核技術論述,呼應用戶「調色盤卡片不加,往技術硬核方向走」的裁定。
  - **卡4 特效 + 卡片置中**(2026-07 後續,用戶:「+特效,順便卡片置中一下,不然略突兀」):卡4 掛上 `fxCard` 特效系統(`#fxRw`),敘事=**「切下去的一刀被時間抽回」**——斬痕畫入(-18°,借卡1視覺語言)→停格→倒轉抽回(scaleX 1→0 收回起點),配三圈**收縮漣漪**(`.rr`,爆發倒放=scale 5.4→0.12 向內收)+**逆時針時鐘**(`.clk`,指針 rotate 0→−720°)+**VHS 倒帶掃描線**(`.scn`,由下往上刷兩次)+回溯啟動瞬間輕震(借卡1 `fxShake` keyframes);紫白色系同前三張,全部動畫 <3.4s 自動卸載窗。**置中**:`.featGrid` 從 `repeat(auto-fit,minmax(260px,1fr))` 改 **flex wrap + justify-content:center**(`.feat{flex:1 1 260px; max-width:370px}`)——4張卡在3欄寬=3+1置中、2欄寬=2+2對稱、手機單欄;桌面寬螢幕下四張寬度趨近一致(~356 vs 370 cap),中等寬度落單卡會比同排略寬,可接受。驗證:凍結動畫截圖(斬痕停格/漣漪+時鐘中格)、375px 無橫捲、reduced-motion 由既有 `.fxCard` 規則覆蓋、console 零錯誤。
- [x] **預告片開場運鏡(Blender 一次性算圖)**(2026-07):`machine_model/intro_flythrough.py`(獨立疊加腳本,**不動 build_machine.py / 原 blend**,結果另存 `intro_flythrough.blend`)。內容:精靈引導視角 Bezier 螺旋收近運鏡(Follow Path+領先 Empty TRACK_TO,末端方位對齊 Key 燈方向,135fr 到位後定格 0.5s 方便剪接時接遊戲標題定鏡)+魔法粒子螺旋(紫→金 ametrine 漸層發光管,以世界 Z 高度映射 ColorRamp;bevel 生長動畫與運鏡同步收尾;方位錨點刻意路過地面魔法陣→RuneRing(z0.1035,θ180°)→CarriageRune(z0.282,θ354°)做「由下往上灌注」;沿線 240 顆發光塵粒,出生順序=頂點序跟著生長點亮)+研磨盤組自轉/水滴循環(blend 內機件原本靜態,只有 mc_gold/mc_purple 材質閃爍動畫)。EEVEE 1920×1080@30fps 150fr,PNG 序列輸出 `machine_model/intro_frames/`(**已 gitignore**,含 `intro_preview.mp4` 快速預覽;DaVinci 剪接直接匯入 PNG 序列)。重跑:`blender -b gem_faceting_machine.blend --factory-startup -P intro_flythrough.py -- anim`(測試幀用 `-- still 1 75 150`;整段約 6 分鐘)。差分版 `intro_flythrough_tight.py`(Sonnet 開的,僅改終點構圖 r=1.32/z=0.66 更近,其餘照抄)輸出到 `intro_frames_tight/`——**用戶已裁定採用 tight 版**當預告片開場;要再調構圖只動檔頭 `END_R`/`END_Z` 兩個數字。**Blender 5.1 API 雷**(這輪踩的):①slotted actions——`action.fcurves` 沒了,要走 `action.layers[0].strips[0].channelbag(slot)`,腳本內建 `anim_fcurves()` helper;②Compositor 走 `scene.compositing_node_group`(node group+interface socket),Glare 參數全 socket 化(`inputs['Type'].default_value='BLOOM'`,沒有 `glare_type` 屬性);③影片輸出要先 `image_settings.media_type='VIDEO'` 否則 file_format enum 裡沒有 FFMPEG;④空的 `strips` collection 是 falsy,`getattr(se,'strips',None) or se.sequences` 這種 fallback 會炸,要用 hasattr;⑤單幀 still 要渲染粒子時,得先從第 1 幀 `frame_set` 逐幀走到目標幀暖快取。
...
- [x] **附魔轉色雙色漸層**(2026-07,用戶:「landing page 用紫黃晶,遊戲內附魔卻沒雙色碧璽系」):`TINTS` 加 6 個漸層項(紫黃晶/粉綠碧璽/藍綠碧璽/藍紫晶/日落/煙墨),頂點色實作+四方向控制+顏色對調,細節見第 6 節「附魔轉色」列。驗證:漸層渲染/方向循環/對調/縮圖/hero/單色回退/還原原色/回溯共存/五份圖紙迴歸 100%/EN 切換/手機排版,全過。
  - **調色盤 tooltip 語言修正**(2026-07,用戶回報:EN 模式 hover 調色盤點還是顯示中文名,新舊 18 顆都一樣):根因=`TINTS` 每項原本只有一個中文 label,`title` 屬性直接寫死那個字串,`buildTints()` 只在頁面載入時跑一次,`applyLang()` 切語言時完全沒碰它。修法=`TINTS` 每項加第二欄英文名(`[zh,en,value]`,漸層 6 項同步補上,如 `雙色碧璽 粉綠`→`Watermelon Tourmaline`),`title` 改用 `tx(t[0],t[1])`;`applyLang()` 補一段迴圈,依 `data-i` 重新設每顆 `.tintDot` 的 `title`,連帶漸層控制列的「漸層方向/顏色對調」兩顆按鈕 tooltip 也一併換語言(同一顆bug)。驗證:18 顆 tooltip 中英即時互換、漸層背景渲染不受影響、曝光滑桿無迴歸。
- [x] **環境亮度滑桿(手動曝光)**(2026-07,用戶回報淺色附魔在教堂光下過曝、但也喜歡調亮的夢幻感 → 給玩家自己調):左欄 `#expBtn`/`#expRow`(滑桿 40%–150% + ↺ 回 100%),控制主場景+導覽器的 `toneMappingExposure`,存 `localStorage gemcraft.exposure`。**實作前提**:renderer/topR 的 toneMapping 從預設 NoToneMapping 改 `LinearToneMapping`(exposure=1.0 時像素級同觀感,只是讓 exposure 生效),**必須在第一次 render 前設好**,不然要全場景 material.needsUpdate;結算 hero/縮圖走 shotR 自己的 ACES(0.85) 完全不受影響——「切割畫面過曝但結算正常」本來就是兩套曝光。驗證:readPixels 數石頭區死白像素 100%=237 → 60%=29 → 140%=2031(夢幻向),重載持久化,五份圖紙迴歸 100%。
